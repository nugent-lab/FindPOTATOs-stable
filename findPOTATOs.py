...
for m in np.arange(len(image_series_list)):
    ...
    source_dataframes = {
        f"df{n}": pd.read_csv(input_directory + order_frames["name"].iloc[n])
        for n in range(len(order_frames))
    }
    print(
    ...
    for name, df in source_dataframes.items():
        if "ml_probs" not in df.columns:
            ...
            print(
                "Removed detections with a ml-assigned probability of being point sources less than:",
                ml_thresh,
                "in",
                name,
            )
            print(
                "Fraction",
                np.round(len(source_dataframes[name]) / old_size, 2),
                "remaining.",
            )
    ...
    for key, df in source_dataframes_moving.items():
        if df.empty:
            print(
                "One of the frames has no sutiable sources. Skipping to next iteration."
            )
    ...
    tree_a = BallTree(
        df0[["RA", "Dec"]], leaf_size=5, metric="pyfunc", func=angular_separation_metric
    )
        ...
        for j in range(len(indicies_b[i])):
            ...
            if distances_b[i][j] > min_dist_deg / 3600:
                ...
                row_data = {
                    ...
                    "observatory_code": [
                        df0["observatory_code"][indicies_b[i][j]],
                        df1["observatory_code"][i],
                    ],
                    ...
                    "decimal_time": [
                        order_frames.decimal_time[0],
                        order_frames.decimal_time[1],
                    ],
                }
                if export_ades:
                    ades_data = {
                        "mag_err": [
                            df0["mag_err"][indicies_b[i][j]],
                            df1["mag_err"][i],
                        ],
                        "RA_err": [df0["RA_err"][indicies_b[i][j]], df1["RA_err"][i]],
                        "Dec_err": [
                            df0["Dec_err"][indicies_b[i][j]],
                            df1["Dec_err"][i],
                        ],
                    }
    ...
    while number_links < len(order_frames):
        ...
        old_time_interval_s = (
            order_frames.time[number_links - 1] - order_frames.time[number_links - 2]
        ).sec
        # time between this frame and the next one
        next_time_interval_s = (
            order_frames.time[number_links] - order_frames.time[number_links - 1]
        ).sec
        ...
        tree_next = BallTree(
            next_sources_df[["RA", "Dec"]],
            leaf_size=5,
            metric="pyfunc",
            func=angular_separation_metric,
        )
        for key, tracklet_df in tracklets.items():  # iterate over tracklet set
            if len(tracklet_df) == number_links:
                ...
                predict_c = temp_coord_b.spherical_offsets_by(
                    c_ra * time_ratio, c_dec * time_ratio
                )
                ...
                for j in range(check_empty[0]):
                    if check_empty[0] == 0:
                        ...
                        new_link = {
                            ...
                            "dec": next_sources_df["Dec"][found_index],
                            "mag": current_mag,
                            "observatory_code": next_sources_df["observatory_code"][
                                found_index
                            ],
